...
Eigen::VectorXd ur5_IK1(const mjModel* m,mjData* d,Eigen::MatrixXd T){
...
bool is_within_limit(const std::vector<double>& joint){
    if(joint.size()){
        for(int i=0; i<6;i++){
...
bool inverse_kinematics(const double *flange_pose, std::vector<std::vector<double>> &slns)
{
	...
	for (int i = 0; i < sign_slns.size(); i++) {
		if (inverse_kinematics(flange_pose, sign_slns[i], 0, sln) && is_within_limit(sln)) {
			slns.push_back(sln);
		}
	}
	return !slns.empty();
...
std::vector<double> vecxd2stdvec(const Eigen::VectorXd& input){
...
Eigen::VectorXd stdvec2vecxd(const std::vector<double>& input){
...
std::vector<double> double2stdvec(const double* input,const int &n){
    std::vector<double> ret;
    ret.clear();
    for(int i=0;i<n;i++){
...
bool convert_sln(const std::vector<double> &joint,std::vector<double> &sln){
    if(!joint.size())
        return false;
...
bool sort_slns(const std::vector<double> &joint,std::vector<std::vector<double>>  &slns){
    std::multimap<double, std::vector<double>> cost_slns;
    std::vector<std::vector<double>> tmp_slns = slns;
    for(auto &sln:slns){
        if(!convert_sln(joint,sln))
            return false;
