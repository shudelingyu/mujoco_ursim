...
//力控
int force_direction[6] = {1, 1, 1, 1, 1, 1}; 
double force_target[6] = {0, 0, 0, 0, 0, 0}; 
double force_gain[6] = {100, 100, 100, 100, 100, 100}; 
double vel_limit[6] = {0.5, 0.5, 0.5, 0.2, 0.2, 0.2}; 
...
void controller(const mjModel* m, mjData* d) {
    ...
    double pm[16], target_pm[16];
    s_pq2pm(pq, pm);
    s_pm_dot_pm(pm_now, pm, target_pm);
    // //
    Eigen::MatrixXd Trance = Eigen::MatrixXd::Zero(4, 4);
    std::cout<<" pm_now "<<std::endl;
    for (int i = 0; i < 4; i++) {
        for (int  j= 0; j < 4; j++)
        {
            std::cout << pm_now[i*4+j] << " ";
            Trance(i,j)=target_pm[i*4+j];
        }
        std::cout<<std::endl;
    }
    ...
    //IK
    Eigen::VectorXd thetaList11=ur5_IK1(m,d,Trance);
    Eigen::VectorXd thetaList=InverseKinematic(pm_now,d);
    std::vector<double> sln;
    select_sln(pm_now,d,sln);
    std::cout<<" target_q00000000000:"<<thetaList.transpose()<<std::endl;
    std::cout<<" target_q44444444444:";
    for(auto &i:sln)
        std::cout<<i<<"  ";
    std::cout<<std::endl;
    ...
    // std::cout<<" target_pm "<<std::endl;
    // for (int i = 0; i < 4; i++) {
    //     for (int  j= 0; j < 4; j++)
    //     {
    //         std::cout << target_pm[i*4+j] << " ";
    //     }
    //     std::cout<<std::endl;
    // }
    //针对零力拖动逆动力学计算
    ...
    for (int i = 0; i < m->nu; i++) {
        ...
        switch (ctrl.mode) {
            ...
            case VELOCITY_CONTROL: {
                ...
                ctrl.current_pos=d->qpos[i];
                d->qpos[i] = ctrl.current_pos;
                // d->qvel[i]=PDcontroller(ctrl,dt);
                d->qvel[i]=0;
                d->qacc[i]=0;
                std::cout<< d->qpos[i] <<"  ";
                break;
            ...
            case POSITION_CONTROL: {
                ...
                // d->qpos[i] += PDcontroller(ctrl,dt)*dt;
                // d->qpos[i] = ctrl.target_pos;
                d->qpos[i] = qinit[i];
                d->qvel[i]=0;
                d->qacc[i]=0;
                std::cout<< ctrl.current_pos <<"  ";
    ...
    // {
    //   d->qpos[i] = thetaList11[i];
    //   d->qvel[i]=0;
    //   d->qacc[i]=0;
    // }
    std::cout<<"d->qpos2222222222222:";
    for(int i=0;i<6;i++)
        std::cout<<d->qpos[i]<<"  ";
    std::cout<<std::endl;
    //target_vel[0]=0.5;
    ...
    // std::cout<<" mujoco_Jac tor "<<std::endl;
    // for (size_t i = 0; i < 6; i++)
    // {
    ...
    // std::cout<<" mujoco_Jac force "<<std::endl;
