...
//double qinit[6] = {0.0, -0.8582831129607316, 1.5085927922538183, -0.6371149901480102, 1.5783361491635128, 0.0};

// double qinit[6] = {0.0, -0.8582831129607316, 1.5085927922538183, 0.02764601535159006, 1.8547963026794125, 0.0};
// double qinit[6] = {M_PI*0.5,M_PI*0.5,M_PI*0.5,M_PI*0.5,M_PI*0.5,0};
//double qinit[6] = {-M_PI*0.5,-M_PI*0.5,0,-M_PI*0.5,0,0};
double qinit[6] = {0,-M_PI*0.5,0,0,M_PI*0.5,0};
...
void controller(const mjModel* m, mjData* d) {
   ...
   //mju_copy(d->qvel, target_vel, m->nv);
    ...
    for (int i = 0; i < 4; i++) {
        for (int  j= 0; j < 4; j++)
        {
            pm_now[i*4+j] = Tab(i,j);
            std::cout << pm_now[i*4+j] << " ";
        }
        std::cout<<std::endl;
    }
    //double target_pose_pq[7]{0};
   //s_pm2pq(pm_now, target_pose_pq);
    //std::cout<<"step 1 target_pose_pq "<<target_pose_pq[3]<<" "<<target_pose_pq[4]<<" "<<target_pose_pq[5]<<" "<<target_pose_pq[6]<<std::endl;
    std::cout<<"step 1 pos "<<d->qpos[0]<<" "<<d->qpos[1]<<" "<<d->qpos[2]<<d->qpos[3]<<" "<<d->qpos[4]<<" "<<d->qpos[5]<<std::endl;
    std::cout<<"step 1 sensordata pos "<<d->sensordata[18]<<" "<<d->sensordata[19]<<" "<<d->sensordata[20]<<std::endl;
    std::cout<<"step 1 sensordata ee_quat"<<d->sensordata[21]<<" "<<d->sensordata[22]<<" "<<d->sensordata[23]<<" "<<d->sensordata[24]<<std::endl;
    ...
    Eigen::MatrixXd force_in_world = Eigen::MatrixXd::Zero(6, 1);
    Eigen::MatrixXd mujoco_force_in_world = Eigen::MatrixXd::Zero(6, 1);
    for (int  j= 0; j < 3; j++)
    {
        force_in_body(j,0) = torque[j];
        force_in_body(j+3,0) = force[j];

        // force_in_body(j,0) = force[j];
        // force_in_body(j+3,0) = torque[j];

        FT_in_sensor[j]= force[j];
        FT_in_sensor[j+3]= torque[j];
    }
    s_pm_dot_v3(pm_now, FT_in_sensor, FT_in_world);
    s_pm_dot_v3(pm_now, FT_in_sensor + 3, FT_in_world + 3);

    std::cout<<"step 2  FT_in_world:"<<std::endl;
    for (int  j= 0; j < 6; j++)
    {
        force_in_world(j,0)=FT_in_world[j];
        std::cout<<FT_in_sensor[j]<<" "<<FT_in_world[j]<<std::endl;
    }
    std::cout<<std::endl;

    force_to_force(Tab,force_in_body,mujoco_force_in_world);
    // mujoco_force_in_world(0,0)=force_in_world(3,0);
    // mujoco_force_in_world(1,0)=force_in_world(4,0);
    // mujoco_force_in_world(2,0)=force_in_world(5,0);
    // mujoco_force_in_world(3,0)=force_in_world(0,0);
    // mujoco_force_in_world(4,0)=force_in_world(1,0);
    // mujoco_force_in_world(5,0)=force_in_world(2,0);


    std::cout<<std::endl;
    std::cout<<"step 2  mujoco_force_in_world:"<<std::endl;
    for (int  j= 0; j < 6; j++)
    {
        std::cout<<mujoco_force_in_world(j,0)<<"  ";
    }
    std::cout<<std::endl;
    ...
    //3. 计算关节外力矩
    auto tor_mujoco = jac1.transpose() * force_in_world;
    // auto force_in_world=ad_ret.transpose()*force_in_body;

    std::cout<<"step 3 tor_mujoco :"<<std::endl;
    for (int  j= 0; j < 6; j++)
    {
        // if (1<time&&time<7)
        // {
        //     d->ctrl[j]=d->ctrl[j]+tor_mujoco(j,0);
        //     //d->ctrl[j]=d->ctrl[j];
        // }else{
        //     d->ctrl[j]=d->ctrl[j];
        // }
        //std::cout<<"time:"<<time<<" "<<tor_mujoco(j,0)<<" ";
        std::cout<<tor_mujoco(j,0)<<" ";
    }
    std::cout<<std::endl;

    //4. 计算笛卡尔力
    auto ans111=jac1.transpose();
    std::cout<<"step 4 force "<<std::endl;
    Eigen::MatrixXd force1111=pinv(ans111)*tor_mujoco;
    ...
    auto cart_vel2=JacobianSpace*qdot;
    std::cout<<"step 5 JacobianSpace "<<std::endl;
    std::cout<<JacobianSpace<<std::endl;

    std::cout<<"step 5 JacobianSpace linvel "<<std::endl;
    for (size_t i = 0; i < 6; i++)
    {
        std::cout<<"  "<<cart_vel2(i,0);
    }
    std::cout<<std::endl;

    //指数积求外力矩
    auto tor_mujoco_Jac = JacobianSpace.transpose() * mujoco_force_in_world;
    std::cout<<" step 6 tor_mujoco_Jac tor "<<std::endl;
    for (size_t i = 0; i < 6; i++)
    {
        std::cout<<"  "<<tor_mujoco_Jac(i,0);
        //d->qfrc_applied[i]=tor1(i,0);
    }
    std::cout<<std::endl;

    //计算笛卡尔力
    auto ans_jac=JacobianSpace.transpose();
    std::cout<<" step 7 tor_mujoco_Jac force "<<std::endl;
    Eigen::MatrixXd force_jac=pinv(ans_jac)*tor_mujoco_Jac;

    for (size_t i = 0; i < 6; i++)
    {
        std::cout<<"  "<<force_jac(i,0);
        //d->qfrc_applied[i]=tor1(i,0);
    }
    std::cout<<std::endl;
    ...
    std::cout<<"step 8 tor_mujoco :"<<std::endl;
    for (int  j= 0; j < 6; j++)
    {
        if (1<time&&time<7)
        {
            //d->ctrl[j]=d->ctrl[j]+tor_mujoco(j,0);
            d->ctrl[j]=d->ctrl[j]+tor_mujoco_Jac(j,0);
        }else{
            d->ctrl[j]=d->ctrl[j];
        }
        //d->ctrl[j]=d->ctrl[j];
    ...
    // for (int i = 0; i < 6; ++i)
    // {
    //     std::cout<<" bias "<<d->qfrc_bias[i]<<" actuator "<<d->qfrc_actuator[i]<<" passive "<<d->qfrc_passive[i]<<" applied "<<d->qfrc_applied[i]<<" constraint "<<d->qfrc_constraint[i]<<" inverse "<<d->qfrc_inverse[i]<<" "<<d->actuator_force[i]<<std::endl;
    // }
...
int main(int argc, const char** argv)
{
    ...
    while( !glfwWindowShouldClose(window))
    {
        if (pert.active) {
            ...
            //     mj_name2id(m, mjOBJ_BODY, "ee_link"), 
            //     d->qfrc_applied);
