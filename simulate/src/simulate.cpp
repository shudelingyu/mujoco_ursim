...
void Simulate::Sync() {
  ...
  std::vector<double> initial_q = {0, -2.55, -0.785, 0, 0, 0};
  // for (int i = 0; i < m_->nu; ++i) {
  //     common::Controller::getInstance().set_joint_targets(i, initial_q[i]);  // 位置和速度目标
  // }

  int count = d_->time/0.002;
    std::vector<double> cur_joint;
     std::vector<double> tar_joint;
    cur_joint.resize(6,0.0);
    for(int i=0;i<m_->njnt;i++){
        cur_joint[i] = d_->qpos[i];
    }
    ...
    for(int i=0;i<m_->nu;i++){
        common::Controller::getInstance().set_joint_targets(i,tar_joint[i]);
    }
  common::Controller::getInstance().compute_control(m_,d_);
...
void Simulate::LoadOnRenderThread() {
  ...
  this->mnew_ = nullptr;
  this->dnew_ = nullptr;
  initcontroller(m_,d_);
}

void Simulate::initcontroller(mjModel *model, mjData *data)
{
  QuinticTrajectory planner;
  common::Controller::getInstance().init(model, data);
  std::vector<double> initial_q = {0, -50*DEG_TO_RAD, 50*DEG_TO_RAD, 90*DEG_TO_RAD, 90*DEG_TO_RAD, 0};
  for (size_t i = 0; i < this->m_->nu; i++)
  {
    this->d_->qpos[i] = initial_q[i];
    this->d_->qvel[i] = 0.0;
    this->d_->qacc[i] = 0.0;
  }
  common::Controller::getInstance().configure_joints_mode(common::ControlMode::POSITION);
  // 设置PID参数

  std::vector<double> joint_kpp;
  std::vector<double> joint_kpd;
  std::vector<double> joint_kpi;
  joint_kpp.resize(this->m_->nu, 50);
  joint_kpd.resize(this->m_->nu, 5);
  joint_kpi.resize(this->m_->nu, 0);
  for (int i = 0; i < this->m_->nu; i++)
    common::Controller::getInstance().configure_joint_pid(i, joint_kpp[i], joint_kpd[i], joint_kpi[i], 0, 0);
