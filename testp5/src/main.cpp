...
//                 ctrl.pid.integral += error * m->opt.timestep;
//                 double derivative = (error - ctrl.pid.prev_error) / m->opt.timestep;
//                 ctrl.pid.prev_error = error;
//                 d->ctrl[i] = ctrl.pid.Kp * error + ctrl.pid.Ki * ctrl.pid.integral + ctrl.pid.Kd * derivative;
//                 break;
//             }
                
//             case POSITION_CONTROL: {
//                 double error = ctrl.target_pos - d->qpos[i];
//                 ctrl.pid.integral += error * m->opt.timestep;
//                 double derivative = (error - ctrl.pid.prev_error) / m->opt.timestep;
//                 ctrl.pid.prev_error = error;
//                 d->ctrl[i] = ctrl.pid.Kp * error + ctrl.pid.Ki * ctrl.pid.integral + ctrl.pid.Kd * derivative;
    ...
    for (int i = 0; i < m->nu; i++) {
        ...
        switch (ctrl.mode) {
            ...
            case POSITION_CONTROL: {
                ...
                // d->qvel[i]=1;
                // d->qacc[i]=0;
                //std::cout<<" "<<d->qvel[i]*360/M_PI<<"  "<<(ctrl.target_pos-d->qpos[i])*360/M_PI<<"  fred_force "<<d->qfrc_bias[i]<<" "<<d->qfrc_actuator[i]<<" "<<d->qfrc_passive[i]<<" "<<d->qfrc_applied[i]<<" "<<d->qfrc_constraint[i]<<" "<<d->qfrc_inverse[i]<<" "<<d->actuator_force[i]<<std::endl;
                break;
            }
        }
    }
...
void init_controller(const mjModel* m, mjData* d)
{
  ...
  double joint_Kpp[]={200,200,200,100,100,100};
  double joint_Kpd[]={10,10,10,10,10,10};
