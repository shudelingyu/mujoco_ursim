...
std::vector<TrajectoryPoint> QuinticTrajectory::generate() {
    ...
    // 无过渡段时直接生成
    if (segments_.empty()) return full_trajectory;
...
//    return trajectory;
    ...
    for (size_t seg = 0; seg < segments_.size(); seg++) {
        ...
        auto seg_traj = interpolateSegment(start, end, seg_time, points_per_segment);
        // 避免重复添加终点（下一段的起点）
        if (!full_trajectory.empty()) seg_traj.erase(seg_traj.begin());
...
std::vector<TrajectoryPoint> LinearInterpolator::interpolateSegment(const TrajectoryPoint& start, const TrajectoryPoint& end,
    ...
    for (int i = 0; i < num_points; ++i) {
        ...
        // 第一帧初始化运动学量为0
        // 首帧初始化
        if (i == 0) initPoseKinematics(current_pose);
        else updateKinematics(current_pose, segment.back(), time_step_);
...
std::vector<TrajectoryPoint> LinearInterpolator::interpolateSegmentSCurve(const TrajectoryPoint& start, const TrajectoryPoint& end, double T)
{
   ...
   for (int i = 0; i < num_points; ++i) {
       ...
       // 首帧初始化
       if (i == 0) initPoseKinematics(pose);
       else updateKinematics(pose, segment.back(), 0.002);
...
{
   // 1. 校验数据
   if (times_.size() < 2) throw std::runtime_error("至少需要2个路径点");
...
void SplinePoseInterpolator::computePositionSpline()
{
    ...
    // 构建三对角矩阵（自然边界条件）
    Eigen::VectorXd h(n);
    for (int i = 0; i < n; ++i) h[i] = times_[i+1] - times_[i];
    ...
    for (int dim = 0; dim < 3; ++dim) {
        ...
        for (int i = 0; i <= n; ++i) c_[i][dim] = c_tmp[i];
    }
