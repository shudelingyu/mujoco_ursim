...
namespace common
{
    MuJoCoViewer::MuJoCoViewer(MujocoContext *mjWrapper)
    ...
    {
        config_ = Controller::getInstance().get_config();
    ...
    {
        double endtime = config_.sim_duration;
    ...
    void MuJoCoViewer::keyboardCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
    {
    ...
    void MuJoCoViewer::drawJointAngles(const mjrRect& viewport) {
        // 获取控制器实例
        auto& controller = Controller::getInstance();
        RobotControlState* state = controller.get_control();
        
        // 创建显示字符串
        std::string jointInfo = "Joint Angles:\n";
        
        for (size_t i = 0; i < state->joints.size(); ++i) {
            std::ostringstream oss;
            const auto& joint = state->joints[i];
            oss << std::fixed << std::setprecision(2) << (joint.current_position * 180 / M_PI);
            jointInfo += "J" + std::to_string(i) + ": " +
                         oss.str() + " deg\n";
        }
        
        // 绘制到右上角
        mjr_overlay(mjFONT_NORMAL, mjGRID_TOPRIGHT, viewport, 
                    "", jointInfo.c_str(), &context_);
    }
}